...
% Library for commonly used nifti utilities.
    ...
    methods (Static, Access = public)    
        ...
        function xform_RAS = get_voxel_RAS_xform(nii_path)
            % Gets the transformation used to convert the nifti file into 
            % "voxel RAS orientation". I used the same method that xform_nii()
            % uses, except the tolerance is set to "1" by default here.
            nii_hdr = load_untouch_header_only(nii_path);       
            ...
            if nii_hdr.hist.qform_code > 0
                ...
                % Scale by pixdim to finish R
                q = nii_hdr.dime.pixdim(1); 
                R = R * diag([nii_hdr.dime.pixdim(2) nii_hdr.dime.pixdim(3) q * nii_hdr.dime.pixdim(4)]);                
            elseif nii_hdr.hist.sform_code > 0
                % https://brainder.org/2012/09/23/the-nifti-file-format/
                R = [nii_hdr.hist.srow_x(1:3);
                     nii_hdr.hist.srow_y(1:3);
                     nii_hdr.hist.srow_z(1:3)];
            else
                % TODO: maybe add code to handle this?
                error('Analyze format not supported');
            end
            
            % Determine transform from R. Clear everything below the 3rd
            % largest absolute value. Not sure if this is the best method
            % but it's what is used in xform_nii.
            xform_RAS = R;
            [~,idx_sort] = sort(abs(xform_RAS(:)));
            xform_RAS(idx_sort(1:end-3)) = 0;
            xform_RAS = sign(xform_RAS);
