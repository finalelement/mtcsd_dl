function vis_vishabyte_multi_shell_shore_preds_decayed_fod_non_neg


    load('D:\Users\Vishwesh\PycharmProjects\shore_mapmri\dl_results\Non_Neg_Shore_input_Shore_decayed_output_v2\Vishabyte_Preds\result.mat')
    ...
    vish_full_q_space = zeros(78,93,75,400);
        ...
        for j = 1:dims(2)
            for k = 1:dims(3)
                if (mask(i,j,k) == 1)
                    ...
                    vish_q_space(i,j,k,:) = exp(q_space_pred_row(301:400));
                    
                    vish_full_q_space(i,j,k,:) = exp(q_space_pred_row);
                end
            end
        end
    end
    ...
    back_ground(back_ground>0.01) = 0.01;
    pred_sh = pred_sh * 100;
    ...
    'sh_coefs', ...    
    {10,60,true});
    dv_single.plot_slice(43,'axial','slice');
    axis image;
